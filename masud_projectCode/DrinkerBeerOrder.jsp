...
<% 
	...
	try{
		...
		ArrayList<Map<String,Integer>> list = new ArrayList();
   		...
   		//Make a query
   		String query = "" ;
   	   	query = "SELECT transactions.item, sum(quantity) FROM transactions LEFT JOIN bills ON transactions.bill_id = bills.bill_id WHERE transactions.type = 'beer' AND drinker = '"+graphType+"' GROUP BY item ORDER BY sum(quantity) DESC";
   		...
   		while (result.next()) { 
   			map=new HashMap<String,Integer>();
   	   			map.put(result.getString("transactions.item"),result.getInt("sum(quantity)"));
        ...
        //Create the chart title according to what the user selected
            ...
            chartTitle = "Most Popular Beers ordered by Drinker";
